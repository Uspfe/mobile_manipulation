...
h = np.arange(start=-1, stop=1, step=0.01)
"""
    print("RBFNew Call {}".format(timeit.timeit(
        "RBFNew.B_fcn(h, mu, zeta)", setup=setup, number=1000)))

    print("RBF Call {}".format(timeit.timeit(
        "RBF.B_fcn(np.where(h<zeta, 1, 0),h, mu, zeta)", setup=setup, number=1000)))
...
def testMPCCosts(config):
    ...
    from mmseq_control.MPCCostFunctions import ControlEffortCostFunciton,EEPos3CostFunction,BasePos2CostFunction
...
def timeMPCCosts(config):
    from mmseq_control.robot import  CasadiModelInterface
    from mmseq_control.MPCCostFunctions import ControlEffortCostFunciton,ControlEffortCostFuncitonNew,EEPos3CostFunction,BasePos2CostFunction,SumOfCostFunctions
    dt = 0.1
    N = 10
    # robot mdl
    casadi_model_interface = CasadiModelInterface(config["controller"])
