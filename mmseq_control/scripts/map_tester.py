    ...
    while not rospy.is_shutdown():
        ...
        if is_map_updated:
            ...
            if False:
                # use tsdf range
                pts = np.around(
                    np.array([np.array([p.x, p.y]) for p in tsdf]), 2
                ).reshape((len(tsdf), 2))
                xs = pts[:, 0]
                ys = pts[:, 1]
                x_lim = [min(xs), max(xs)]
            ...
            # x-y
            sdf.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[0.1, 0.1], block=False)
            sdf.vis(x_lim=x_lim, y_lim=y_lim, z_lim=[0.7, 0.7], block=True)
            # sdf.vis(x_lim=x_lim,
            #         y_lim=y_lim,
            #         z_lim=[1.3, 1.3],
            #         block=False)
...
def test_2d(config):
    ...
    while not rospy.is_shutdown():
        ...
        if map_ros_interface.valid:
            sdf.vis(x_lim=x_lim, y_lim=y_lim, block=True)
        else:
            print("map invalid")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--map2d", action="store_true", help="Test SDF3DNew")
    parser.add_argument("--map3d", action="store_true", help="Test SDF2DNew")
