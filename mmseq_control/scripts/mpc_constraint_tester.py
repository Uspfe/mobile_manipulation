...
from mmseq_control_new.MPCConstraints import SignedDistancenConstraint
...
def testSDF2DMapConstraint(config):
    ...
    casadi_model_interface.sdf_map.query_val(X.flatten(), Y.flatten())
    t1 = time.perf_counter()
    print(f"Map Query eval {t1-t0}")
    ...
    parms = casadi_model_interface.sdf_map.get_params()
    t0 = time.perf_counter()
    _, g_sdf = const.check(x_bar, u_bar, *parms)
    t1 = time.perf_counter()
    print(f"SDF constraint eval {t1-t0}")
    ...
    J_soft = const_soft.evaluate_vec(x_bar, u_bar,*parms)
...
def testSDF3DMapConstraint(config):
    ...
    casadi_model_interface = CasadiModelInterface(config["controller"])
    _, map = tsdf_map_interface.get_map()
    casadi_model_interface.sdf_map.update_map(*map)
    tsdf, tsdf_vals = tsdf_map_interface.tsdf, tsdf_map_interface.tsdf_vals
    ...
    X,Y= np.meshgrid(qbs_x, qbs_y)
    t0 = time.perf_counter()
    casadi_model_interface.sdf_map.query_val(X.flatten(), Y.flatten(), np.ones_like(X).flatten()*0.5)
    t1 = time.perf_counter()
    casadi_model_interface.sdf_map.query_grad(X.flatten(), Y.flatten(), np.ones_like(X).flatten()*0.5)
    t2 = time.perf_counter()
    print(f"Map Query val {t1-t0}")
    print(f"Map Query grad {t2-t1}")
    ...
    mu = config["controller"]["collision_soft"]['sdf']["mu"]
    zeta = config["controller"]["collision_soft"]['sdf']["zeta"]
    const_soft = SoftConstraintsRBFCostFunction(mu, zeta, const, "SDFCollisionSoftConstraint",expand=False)
    # J_soft = [const_soft.evaluate(x_bar[i,:], u_bar)/X.size for i in range(N+1)]
    t0 = time.perf_counter()
    J_soft = const_soft.evaluate_vec(x_bar, u_bar,*parms)
    t1 = time.perf_counter()
    _, g_sdf = const_soft.quad(x_bar, u_bar, *parms)
    t2 = time.perf_counter()
    print(f"SDF Soft Cost Eval {t1-t0}")
    print(f"SDF Soft Cost Quad {t2-t1}")
...
if __name__ == "__main__":
    ...
    rospy.init_node('mpc_constraint_tester')
    
    testSDF3DMapConstraint(config)