...
def testSDF2DMapConstraint(config):
    with rosbag.Bag("/home/tracy/PhD/data/map/cube_map.bag", "r") as bag:
        for topic, msg, t in bag.read_messages(
            topics=["/pocd_slam_node/occupied_ef_dist_nodes"]
        ):
    ...
    x_bar[:, :2] = qbs
    d_safe = 0.15
    const = SignedDistanceCollisionConstraint(
        robot_mdl,
...
def testSDF3DMapConstraint(config):
    ...
    X, Y = np.meshgrid(qbs_x, qbs_y)
    t0 = time.perf_counter()
    casadi_model_interface.sdf_map.query_val(
        X.flatten(), Y.flatten(), np.ones_like(X).flatten() * 0.5
    )
    t1 = time.perf_counter()
    casadi_model_interface.sdf_map.query_grad(
        X.flatten(), Y.flatten(), np.ones_like(X).flatten() * 0.5
    )
    ...
    zeta = config["controller"]["collision_soft"]["sdf"]["zeta"]
    const_soft = SoftConstraintsRBFCostFunction(
        mu, zeta, const, "SDFCollisionSoftConstraint", expand=False
    )
    # J_soft = [const_soft.evaluate(x_bar[i,:], u_bar)/X.size for i in range(N+1)]
    t0 = time.perf_counter()
    J_soft = const_soft.evaluate_vec(x_bar, u_bar, *parms)
