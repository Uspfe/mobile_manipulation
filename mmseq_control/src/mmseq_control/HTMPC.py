...
from mmseq_control.MPCConstraints import (
    HierarchicalTrackingConstraint,
    HierarchicalTrackingConstraintCostValue,
    MotionConstraint,
    StateControlBoxConstraint,
    StateControlBoxConstraintNew,
    SignedDistanceCollisionConstraint,
    EEUpwardConstraint,
)
...
class MPC:
    def __init__(self, config):
        ...
        self.EEPos3Cost = EEPos3CostFunction(
            self.dt, self.N, self.robot, config["cost_params"]["EEPos3"]
        )
        self.EEPos3BaseFrameCost = EEPos3BaseFrameCostFunction(
            self.dt, self.N, self.robot, config["cost_params"]["EEPos3"]
        )
        self.BasePos2Cost = BasePos2CostFunction(
            self.dt, self.N, self.robot, config["cost_params"]["BasePos2"]
        )
        if self.params["penalize_du"]:
            self.CtrlEffCost = ControlEffortCostFuncitonNew(
                self.dt, self.N, self.robot, config["cost_params"]["Effort"]
            )
        else:
            self.CtrlEffCost = ControlEffortCostFunciton(
                self.dt, self.N, self.robot, config["cost_params"]["Effort"]
            )
        ...
        if self.params["penalize_du"]:
            self.xuCst = StateControlBoxConstraintNew(
                self.dt, self.robot, self.N, tol=1e-5
            )
        else:
            self.xuCst = StateControlBoxConstraint(
                self.dt, self.robot, self.N, tol=1e-5
            )
        ...
        for name in self.collision_link_names:
            ...
            sd_cst = SignedDistanceCollisionConstraint(
                self.robot,
                sd_fcn,
                self.dt,
                self.N,
        ...
        self.eeUpwardCst = EEUpwardConstraint(
            self.robot, self.params["ee_upward_deviation_angle_max"], self.dt, self.N
        )

        self.xuSoftCst = SoftConstraintsRBFCostFunction(
            self.params["xu_soft"]["mu"],
            self.params["xu_soft"]["zeta"],
            self.xuCst,
            "xuSoftCst",
        )
        ...
        self.eeUpwardSoftCst = SoftConstraintsRBFCostFunction(
            self.params["ee_upward_soft"]["mu"],
...
class HTMPCSQP(MPC):
    def __init__(self, config):
        ...
        self.MotionCst = MotionConstraint(
            self.dt, self.N, self.robot, "DI Motion Model"
        )
        self.hierarchy_cst_type = getattr(
            MPCConstraint, self.params["hierarchy_constraint_type"]
        )
    ...
    def control(self, t, robot_states, planners, map=None):
        ...
        for id, planner in enumerate(planners):
            ...
            if (
                planner.type == "EE"
                and planner.ref_data_type == "Vec3"
                and (
                    planner.__class__.__name__ == "EESimplePlanner"
                    or planner.__class__.__name__ == "EEPosTrajectoryLine"
                )
            ):
                ...
                planner.type == "EE"
                and planner.ref_data_type == "Vec3"
                and planner.__class__.__name__ == "EESimplePlannerBaseFrame"
            ):
            ...
            if id < num_plans - 1:
                hier_csts.append(
                    self.hierarchy_cst_type(cost_fcns[-1][0], planner.type)
                )
        ...
        t0 = time.perf_counter()
        xbar_opt, ubar_opt = self.solveHTMPC(
            xo, self.x_bar.copy(), self.u_bar.copy(), cost_fcns, hier_csts, r_bars
        )
        t1 = time.perf_counter()
    ...
    def solveHTMPC(self, xo, xbar, ubar, cost_fcns, hier_csts, r_bars):
        ...
        self.cost_iter = np.zeros(
            (self.params["HT_MaxIntvl"], task_num, self.params["ST_MaxIntvl"] + 1)
        )
        self.cost_final = np.zeros(task_num)

        self.step_size = np.zeros(
            (self.params["HT_MaxIntvl"], task_num, self.params["ST_MaxIntvl"])
        )
        self.solver_status = np.zeros_like(self.step_size)

        self.tol_schedule = np.linspace(
            self.params["hierarchy_const_tol"],
            self.params["hierarchy_const_tol"],
            self.params["HT_MaxIntvl"],
        )
            ...
            for task_id, cost_fcn in enumerate(cost_fcns):
                if self.params["soft_cst"]:
                    ...
                    for name, soft_cst in self.collisionSoftCsts.items():
                        ...
                        if name == "sdf":
                            st_cost_fcn_params += [
                                self.model_interface.sdf_map.get_params()
                            ]
                        else:
                            st_cost_fcn_params += [[]]
                ...
                if task_id > 0:
                    ...
                    for prev_task_id in range(task_id):
                        if (
                            self.params["hierarchy_constraint_type"]
                            == "HierarchicalTrackingConstraint"
                        ):
                            csts_params["ineq"].append(
                                [r_bars[prev_task_id][0][0].T, e_bars[prev_task_id]]
                            )
                        elif (
                            self.params["hierarchy_constraint_type"]
                            == "HierarchicalTrackingConstraintCostValue"
                            ...
                            csts_params["ineq"].append(
                                [r_bars[prev_task_id][0][0].T, J_bars[prev_task_id]]
                            )

                t0 = time.perf_counter()
                xbar_lopt, ubar_lopt, status = self.solveSTMPCCasadi(
                    xo,
                    xbar_l.copy(),
                    ubar_l.copy(),
                    st_cost_fcn,
                ...
                if task_id < task_num - 1:
                    e_bars_l = cost_fcn[0].e_bar_fcn(
                        xbar_lopt.T, ubar_lopt.T, r_bars[task_id][0][0].T
                    )
                    e_bars.append(e_bars_l.toarray().flatten())

                    J_bar_l = cost_fcn[0].J_bar_fcn(
                        xbar_lopt.T, ubar_lopt.T, r_bars[task_id][0][0].T
                    )
        ...
        for task_id, cost_fcn in enumerate(cost_fcns):
            self.cost_final[task_id] = self._eval_cost_functions(
                cost_fcn, xbar_l, ubar_l, r_bars[task_id]
            )
            # if self.params["soft_cst"]:
            #     self.cost_final[task_id] += self.xuSoftCst.evaluate(xbar_l, ubar_l, *xuSoft_param[0])
        return xbar_l, ubar_l

    def solveSTMPCCasadi(
        self,
        xo,
        xbar,
        ubar,
        ...
        xbar_i = xbar.copy()
        ubar_i = ubar.copy()
        self.cost_iter[ht_iter, task_id, 0] = self._eval_cost_functions(
            cost_fcn, xbar_i, ubar_i, cost_fcn_params
        )
        ...
        for i in range(self.params["ST_MaxIntvl"]):
            ...
            if results["status"] == "optimal":
                linesearch_step_opt, _ = self.lineSearchNew(
                    xo,
                    ubar_i,
                    dubar,
                    cost_fcn,
    ...
    def scaleConstraints(self, G, h):
        ...
        #     print(1)
        G[largeval_idx] = G[largeval_idx] / np.abs(h[largeval_idx]).reshape(
            (len(largeval_idx[0]), 1)
        )
        h[largeval_idx] = 1.0 * (h[largeval_idx] > 0) - 1.0 * (h[largeval_idx] < 0)
    ...
    def checkLinConstraintsFeas(self, G, h, A, b, dz, tol=1e-2):
        ...
        if len(ineq_vio_indx[0] > 0):
            self.py_logger.log(
                10,
                "Linearized Inequality Constraints # %d infeasible!",
                ineq_vio_indx[0],
            )
            feas = False
        ...
        if len(eq_vio_indx[0] > 0):
            self.py_logger.log(
                10,
                "Linearized Equality Constraints #{} infeasible!".format(
                    eq_vio_indx[0]
                ),
            )
            feas = False
    ...
    def lineSearchNew(
        self, xo, ubar, dubar, cost_fcn, cost_fcn_params, csts, csts_params, dzopt
    ):
        ...
        for k in range(MAX_ITER):
            ...
            for cst_id, cst in enumerate(csts["ineq"]):
                ...
                if not feas_i:
                    ...
                    self.py_logger.log(
                        10,
                        "Controller: line search step {} not feasible. Violating constraint {}".format(
                            t, cst.name
                        ),
                    )
                    break
            ...
            if feas:
                ...
                for fid, f in enumerate(cost_fcn):
                    ...
                    J_xp_lin += (
                        f.evaluate(xbar, ubar, *cost_fcn_params[fid])
                        + alpha * t * g.T @ dzopt
                    )
                    ...
                    self.py_logger.log(
                        15,
                        f"Line Search Func {f.name} J{t1-t0}, Jquad{t2-t1}, Jlin{t3-t2}",
                    )
                ...
                if J_xp < J_xp_lin:
                    ...
                    self.py_logger.log(
                        10,
                        "Controller: line search step acceptance condition not met {}.".format(
                            J_xp_lin - J_xp
                        ),
                    )
...
class HTMPCLex(HTMPCSQP):
    ...
    def solveHTMPC(self, xo, xbar, ubar, cost_fcns, hier_csts, r_bars):
        ...
        for i in range(self.params["HT_MaxIntvl"]):
            for task_id, cost_fcn in enumerate(cost_fcns):
                ...
                xbar_lopt, ubar_lopt, status = self.solveSTMPC(
                    xo,
                    xbar_l.copy(),
                    ubar_l.copy(),
                    cost_fcn,
    ...
    def solveSTMPC(
        ...
        self.cost_iter[ht_iter, task_id, 0] = self._eval_cost_functions(
            cost_fcn, xbar, ubar, cost_fcn_params
        )
        # f (x)
        f_eqn = cs.DM.zeros()
        for cost_id, cost in enumerate(cost_fcn):
            if cost.__class__.__name__ != "ControlEffortCostFunciton":
                J_eqn = cost.J_fcn(
                    self.x_bar_sym, self.u_bar_sym, cost_fcn_params[cost_id][0].T
                )
            else:
                Jxu_eqn = cost.xu_cost.J_fcn(
                    self.x_bar_sym, self.u_bar_sym, np.zeros(self.QPsize)
                )
                Jdu_eqn = cost.du_cost.J_fcn(
                    self.x_bar_sym,
                    self.u_bar_sym,
                    np.hstack(
                        (cost_fcn_params[cost_id][0], np.zeros(self.nu * (self.N - 1)))
                    ),
                )
        ...
        for cst_id, cst in enumerate(csts["eq"]):
            h_eqn = cst.h_fcn(
                self.x_bar_sym, self.u_bar_sym, *csts_params["eq"][cst_id]
            )
        ...
        for cst_id, cst in enumerate(csts["ineq"]):
            gi_eqn = cst.g_fcn(
                self.x_bar_sym, self.u_bar_sym, *csts_params["ineq"][cst_id]
            )
        ...
        self.cost_iter[ht_iter, task_id, 1] = self._eval_cost_functions(
            cost_fcn, xbar_new, ubar_new, cost_fcn_params
        )
...
class HTMPCSQPNEW(MPC):
    def __init__(self, config):
        ...
        self.st_cost_EEPos3 = SumOfCostFunctions(
            [self.EEPos3Cost] + self.st_cost_fcns_common
        )
        self.st_cost_EEPos3BaseFrameCost = SumOfCostFunctions(
            [self.EEPos3BaseFrameCost] + self.st_cost_fcns_common
        )
        self.st_cost_BasePos2Cost = SumOfCostFunctions(
            [self.BasePos2Cost] + self.st_cost_fcns_common
        )
        ...
        for id, planner in enumerate(planners):
            if (
                ...
                self.py_logger.warning(
                    f"unknown cost type, planner {planner.__class__.__name__}"
                )
            ...
            if id < num_plans - 1:
                hier_csts.append(
                    self.hierarchy_cst_type(
                        ht_cost_fcns[-1].cost_fcn_obj[0], planner.type
                    )
                )
    ...
    def solveHTMPC(self, xo, xbar, ubar, cost_fcns, cost_params, hier_csts):
        ...
        for i in range(self.params["HT_MaxIntvl"]):
            ...
            for task_id, cost_fcn in enumerate(cost_fcns):
                csts = (
                    {"eq": [self.MotionCst], "ineq": []}
                    if self.params["soft_cst"]
                    else {"eq": [self.MotionCst], "ineq": [self.xuCst]}
                )
                ...
                if task_id > 0:
                    ...
                    for prev_task_id in range(task_id):
                        if (
                            ...
                            csts_params["ineq"].append(
                                [cost_params[prev_task_id][0].T, e_bars[prev_task_id]]
                            )
                        elif (
                            self.params["hierarchy_constraint_type"]
                            == "HierarchicalTrackingConstraintCostValue"
                            ...
                            csts_params["ineq"].append(
                                [cost_params[prev_task_id][0].T, J_bars[prev_task_id]]
                            )
                ...
                if task_id < task_num - 1:
                    ...
                    e_bars_l = tracking_cost_fcn.e_bar_fcn(
                        xbar_lopt.T, ubar_lopt.T, cost_params[task_id][0].T
                    )
                    e_bars.append(e_bars_l.toarray().flatten())

                    J_bar_l = tracking_cost_fcn.J_bar_fcn(
                        xbar_lopt.T, ubar_lopt.T, cost_params[task_id][0].T
                    )
                    J_bars.append(J_bar_l)
    ...
    def solveSTMPCCasadi(
        ...
        for i in range(self.params["ST_MaxIntvl"]):
            ...
            soft_cst_cost_params = self.get_soft_cst_params(xbar_i, ubar_i)
            params = cost_fcn_params + soft_cst_cost_params
            self.cost_iter[ht_iter, task_id, i + 1] = cost_fcn.evaluate(
                xbar_i, ubar_i, *params
            )
            self.step_size[ht_iter, task_id, i] = linesearch_step_opt
            self.solver_status[ht_iter, task_id, i] = results["status_val"]
    ...
    def lineSearchNew(
        ...
        for k in range(MAX_ITER):
            ...
            if feas:
                ...
                J_xp_lin = (
                    cost_fcn.evaluate(xbar, ubar, *params) + alpha * t * g.T @ dzopt
                )
