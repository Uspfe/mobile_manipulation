...
class STMPCSQP(MPC):
    def __init__(self, config):
        ...
        self.BasePose3Cost = BasePose3CostFunction(
            self.dt, self.N, self.robot, config["cost_params"]["BasePos2"]
        )
        self.ArmJointCost = ArmJointCostFunction(
            self.dt, self.N, self.robot, config["cost_params"]["ArmJoint"]
        )
    ...
    def control(self, t, robot_states, planners):
        ...
        self.cost_final = self._eval_cost_functions(
            cost_fcns, xbar_lopt, ubar_lopt, r_bars
        )
        if planner.type == "EE":
    ...
    def solveSTMPCCasadi(
        ...
        self.cost_iter[0] = self._eval_cost_functions(
        ...
        for i in range(self.params["ST_MaxIntvl"]):
            ...
            self.cost_iter[i + 1] = self._eval_cost_functions(
                cost_fcn, xbar_i, ubar_i, cost_fcn_params
            )
