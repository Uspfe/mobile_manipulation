...
class MotionConstraint(Constraint):
    def __init__(self, dt, N, robot_mdl, name="Motion Model"):
        ...
        self.grad_fcn = cs.Function(
            "dhdz", [self.z_bar_sym, self.xo_sym], [self.grad_eqn]
        )
    ...
    def _setUpConstraintSymMdl(self):
        ...
        x_next_bar_eqn = fmdl_pred_multishooting_fcn(
            self.x_bar_sym[:, :-1], self.u_bar_sym
        )
        hmdl_eqn = cs.vec(self.x_bar_sym - cs.horzcat(self.xo_sym, x_next_bar_eqn))
        hmdl_fcn = cs.Function(
            "hmdl",
            [self.x_bar_sym, self.u_bar_sym, self.xo_sym],
            [hmdl_eqn],
            ["x_bar", "u_bar", "xo"],
            ["hmdl"],
        )
    ...
    def check(self, x_bar, u_bar, *params):
        ...
        if len(indx) > 0:
            self.py_logger.debug(
                self.name + " Constraint is violated at {}".format(indx)
            )
            return False
        else:
            return True
...
class NonlinearConstraint(Constraint):
    def __init__(self, dt, nx, nu, ng, N, g_fcn, params_sym, constraint_name, tol=1e-5):
        ...
        self.grad_fcn = cs.Function(
            "dgdz_" + constraint_name,
            [self.z_bar_sym] + self.params_sym,
            [self.grad_eqn],
            ["z_bar"] + self.params_name,
            ["dgdz"],
        )
...
class HierarchicalTrackingConstraint(NonlinearConstraint):
    def __init__(self, cost_fcn_obj, name="Hierarchy"):
        ...
        g_eqn = cs.vertcat(e_bar_eqn, -e_bar_eqn) - cs.vertcat(
            e_bar_p_abs_eqn, e_bar_p_abs_eqn
        )
        g_fcn = cs.Function(
            "g_" + name,
            [
                cost_fcn_obj.x_bar_sym,
                cost_fcn_obj.u_bar_sym,
                cost_fcn_obj.r_bar_sym,
                e_bar_p_sym,
            ],
            [g_eqn],
        )
...
class StateControlBoxConstraint(NonlinearConstraint):
    def __init__(self, dt, robot_mdl, N, tol=1e-2):
        ...
        g_eqn = cs.vertcat(self.z_bar_sym, -self.z_bar_sym) - np.expand_dims(
            np.hstack((self.ub, -self.lb)), -1
        )
        g_fcn = cs.Function(
            "g_" + name,
            [self.x_bar_sym, self.u_bar_sym],
            [g_eqn],
            ["x_bar", "u_bar"],
            ["g_" + name],
        )
...
class StateControlBoxConstraintNew(NonlinearConstraint):
    def __init__(self, dt, robot_mdl, N, tol=1e-2):
        ...
        du_eqn = cs.vec(self.u_bar_sym) - cs.vertcat(
            self.u_prev, cs.vec(self.u_bar_sym[:, :-1])
        )
        g_eqn_du = cs.vertcat(du_eqn, -du_eqn) - np.expand_dims(
            np.hstack((self.ub_du, -self.lb_du)), -1
        )

        g_fcn = cs.Function(
            "g_" + name,
            [self.x_bar_sym, self.u_bar_sym, self.u_prev],
            [cs.vertcat(g_eqn, g_eqn_du)],
            ["x_bar", "u_bar", "u_prev"],
            ["g_" + name],
        )
...
class SignedDistanceCollisionConstraint(NonlinearConstraint):
    def __init__(
        self, robot_mdl, signed_distance_fcn, dt, N, d_safe, name="obstacle", tol=1e-2
    ):
        ...
        g_sym_list = [
            -signed_distance_fcn(self.x_bar_sym[:nq, k], *params_sym) + d_safe
            for k in range(N + 1)
        ]
        ...
        g_fcn = cs.Function(
            "g_" + self.name,
            [self.x_bar_sym, self.u_bar_sym] + params_sym,
            [g_sym],
            ["x_bar", "u_bar"] + params_name,
            ["g_collision"],
        )
...
class EEUpwardConstraint(NonlinearConstraint):
    def __init__(self, robot_mdl, psi_max, dt, N, name="EEUpward", tol=1e-2):
        ...
        g_sym_list = [
            -fee(self.x_bar_sym[:nq, k])[1][2, 2] + np.cos(psi_max)
            for k in range(N + 1)
        ]
...
def testCollisionConstraint(config):
    ...
    const_soft = SoftConstraintsRBFCostFunction(
        mu, zeta, const, "SelfCollisionSoftConstraint"
    )
...
def testSoftConstraint(config):
    ...
    cost_params = {}
    cost_params["EE"] = {"Qk": 1.0, "P": 100.0}
    cost_params["base"] = {"Qk": 1.0, "P": 100.0}
    cost_params["effort"] = {
        "Qqa": 0.0,
        "Qqb": 0.0,
        "Qva": 1e-2,
...
def testNonlinearConstraint():
    ...
    g_eqn = cs.vertcat(
        x_bar_sym[x_idx[0], x_idx[1]] * p_sym[0],
        u_bar_sym[u_idx[0], u_idx[1]] * p_sym[1],
    )
