...
def inverse_kynematics_dynamic(
    self, current_end_effector, current_params, future_goals, k=1, t=0.1
):
    """Compute the inverse kinematics for the mobile manipulator."""
    # Compute the inverse kinematics for the mobile manipulator

    qs, intermediate_qs, intermediate_qs_dots = self.new_pose_luoponov_control(
        current_params, future_goals[0], t=t
    )
...
def compute_positions(
    self, starting_configurations, intermediate_points, t_spacing=0.1
):
    ...
    for i in range(1, len(intermediate_points)):
        qs, intermediate_qs, intermediate_qs_dots = self.inverse_kynematics_dynamic(
            intermediate_points[i - 1], qs, intermediate_points[i:], t=t_spacing
        )
        inter_times = np.linspace(
            0, len(intermediate_qs) * t_spacing, len(intermediate_qs)
        )
