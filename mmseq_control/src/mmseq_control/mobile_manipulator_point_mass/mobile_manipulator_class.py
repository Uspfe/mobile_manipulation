...
class MobileManipulatorPointMass(MobileManipulator3D):
    def __init__(self, config):
        ...
        setattr(
            ...
            "inverse_kynematics_dynamic",
            inverse_kynematics.inverse_kynematics_dynamic,
        )
        setattr(
            MobileManipulatorPointMass,
            "compute_positions",
            inverse_kynematics.compute_positions,
        )
        setattr(
            MobileManipulatorPointMass,
            "compute_error",
            inverse_kynematics.compute_error,
        )
        setattr(
            MobileManipulatorPointMass,
            "new_pose_luoponov_control",
            inverse_kynematics.new_pose_luoponov_control,
        )
        from . import end_effector

        setattr(
            MobileManipulatorPointMass,
            "generate_intermediate_points_withouth_trajectory",
            end_effector.generate_intermediate_points_withouth_trajectory,
        )
        from . import trajectory_graph

        setattr(
            MobileManipulatorPointMass, "create_graph", trajectory_graph.create_graph
        )
        from . import q_velocity_generation

        setattr(
            MobileManipulatorPointMass,
            "generate_waypoints_q_dot",
            q_velocity_generation.generate_waypoints_q_dot,
        )
        ...
        setattr(
            ...
            trajectory_computation.calculate_trajectory,
        )
        setattr(
            MobileManipulatorPointMass,
            "calculate_trajectory_simple",
            trajectory_computation.calculate_trajectory_simple,
        )
