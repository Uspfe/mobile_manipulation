...
class EqTask(IDKCTaskBase):
    def __init__(self, e_fcn, nq, nΩ, κ, name="EqualityTask"):
        ...
        self.e_eqn = self.e_fcn(
            self.q_sym, self.Ω_sym
        )  # error function to be driven to zero
        self.dedq_eqn = cs.jacobian(self.e_eqn, self.q_sym)  # error function jacobian
        self.dedΩ_eqn = cs.jacobian(self.e_eqn, self.Ω_sym)
        self.ed_eqn = (
            -self.κ * self.e_eqn - self.dedΩ_eqn @ self.Ωdot_sym
        )  # error function feedback signal (desired twist)

        self.J_fcn = cs.Function(
            "J_" + self.name, [self.q_sym, self.Ω_sym], [self.dedq_eqn]
        )
        self.ed_fcn = cs.Function(
            "ed_" + self.name, [self.q_sym, self.Ω_sym, self.Ωdot_sym], [self.ed_eqn]
        )
...
class CollisionAvoidance(IneqTask):
    def __init__(self, model_interface, params):
        self.collision_link_names = (
            ["self"] if params["self_collision_avoidance_enabled"] else []
        )
        self.collision_link_names += (
            model_interface.scene.collision_link_names["static_obstacles"]
            if params["static_obstacles_collision_avoidance_enabled"]
            else []
        )
        ...
        if len(self.collision_link_names) > 0:
            ...
            super().__init__(
                e_fcn,
                cs.DM.zeros(e_eqn.shape[0]),
                q_sym.shape[0],
                params["collision_avoidance"]["κ"],
                1.0 / params["ctrl_rate"],
                "CollisionAvoidance",
            )
...
class JointAngleBound(IneqTask):
    def __init__(self, robot, params):
        ...
        e_fcn = cs.Function(
            "e_fcn", [robot.q_sym], [cs.vertcat(robot.q_sym, -robot.q_sym)]
        )
...
def test_joint_angle_bound(config):
    ...
    dt = 1.0 / config["controller"]["ctrl_rate"]
    ...
    e = np.hstack((eu, -el))
    print(
        "J diff: {}, e diff: {}".format(
            np.linalg.norm(J_sym - J), np.linalg.norm(e_sym - e)
        )
    )
...
def test_position_tracking(config):
    ...
    print(
        "Jb diff:{}, ed_b diff:{}".format(
            np.linalg.norm(Jbase - Jbase_sym), np.linalg.norm(ed_base - ed_base_sym)
        )
    )
    ...
    print(
        "eb diff: {}, eb_lin diff: {}".format(
            np.linalg.norm(eb - eb_sym), np.linalg.norm(eb_lin - eb_lin_sym)
        )
    )
    ...
    print(
        "Jee diff:{}, ed_ee diff:{}".format(
            np.linalg.norm(Jee - Jee_sym), np.linalg.norm(ed_ee - ed_ee_sym)
        )
    )
    ...
    print(
        "eee diff: {}, eee_lin diff: {}".format(
            np.linalg.norm(eee - eee_sym), np.linalg.norm(eee_lin - eee_lin_sym)
        )
    )
