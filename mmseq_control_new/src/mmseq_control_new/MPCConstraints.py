...
class HierarchicalTrackingConstraint(NonlinearConstraint):
    def __init__(self, cost_fcn_obj: TrajectoryTrackingCostFunction, name="Hierarchy"):
        ...
        e_eqn = cost_fcn_obj.e_fcn(
            self.x_sym, self.u_sym, self.p_struct["r_" + cost_fcn_obj.name]
        )
        ...
        self.g_fcn = cs.Function(
            "g_" + self.name, [self.x_sym, self.u_sym, self.p_struct.cat], [self.g_eqn]
        )
    ...
    def get_p_dict(self, sym=True):
        if self.p_dict is None:
            ...
            for key, val in self.p_dict.items():
                if key == "e_p":
                    p_dict[key + f"_{self.name}"] = (
                        val if sym else cs.DM.zeros(val.shape)
                    )
                else:
