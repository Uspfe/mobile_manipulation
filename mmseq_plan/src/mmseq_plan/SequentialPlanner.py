...
from mmseq_utils.point_mass_computation_scripts.casadi_initial_guess import (
    initial_guess,
    initial_guess_simple,
    slow_down_guess_only_ee,
)
...
from mmseq_utils.plot_casadi_time_optimal import (
    plot_obstacle_avoidance,
    compare_trajectories_casadi_plot,
    plot_motion_model,
)
...
class SequentialPlanner(WholeBodyPlanner):
    def __init__(self, config, motion_class=None, file_path=None):
        if motion_class is not None:
            ...
            possibly_motion_class = getattr(
                MobileManipulatorPointMass, config["motion_class_type"], None
            )
            if possibly_motion_class is None:
                possibly_motion_class = getattr(
                    robot, config["motion_class_type"], None
                )
            if possibly_motion_class is None:
                raise ValueError(
                    "Motion class {} not found".format(config["motion_class_type"])
                )
            ...
            self.self_collision_safety_margin = config["collision_safety_margin"][
                "self"
            ]
            self.ground_collision_safety_margin = config["collision_safety_margin"][
                "ground"
            ]
            # print(self.starting_configuration)
            self.starting_configuration = mm.load_home_position(
                config.get("home", "default")
            )
            self.config = config
    ...
    @classmethod
    def initializeFromMotionClass(
        self, motion_class, file_path=os.path.join(os.getcwd(), "planner_solutions")
    ):
        return SequentialPlanner(None, motion_class, file_path)

    def second_slow_down(
        self,
        start_state,
        end_ee,
        motion_model,
        ...
        X, g, lbg, ubg, lbx, ubx, t = self.setup_single_problem(
            start_state,
            end_ee,
            motion_model,
            forward_kinematic,
            ...
            iteration=0,
            obstacles_avoidance=obstacle_avoidance,
            self_avoidance=self_avoidance,
            end_zero_velocity=True,
        )
        OPT_variables = ca.vertcat(t, X)

    def setup_single_problem(
        self,
        start,
        goal,
        motion_model,
        ...
        for i in range(N - 1):
            ...
            if i == 0:
                if initial_point:
                    ...
                    g.append(
                        ...
                        - start[:u_start_index]
                        - dt
                        * motion_model(start[:u_start_index], start[u_start_index:])
                    )
            ...
            g.append(
                X[:u_start_index, i + 1]
                - X[:u_start_index, i]
                - dt * motion_model(X[:u_start_index, i], X[u_start_index:, i])
            )
            lbg.append(ca.DM.zeros(2 * state_dim))
            ubg.append(ca.DM.zeros(2 * state_dim))
            lbx.append(cur_lbx)
            ...
            if self_avoidance is not None:
                self_avoid = (
                    self_avoidance(X[:vel_start_index, i])
                    - self.self_collision_safety_margin
                )
                g.append(self_avoid)
            ...
            if obstacles_avoidance is not None:
                obs = (
                    obstacles_avoidance(X[:vel_start_index, i])
                    - self.ground_collision_safety_margin
                )
                g.append(obs)
        ...
        if self_avoidance is not None:
            self_avoid = (
                self_avoidance(X[:vel_start_index, N - 1])
                - self.self_collision_safety_margin
            )
            g.append(self_avoid)
            lbg.append(ca.DM.zeros(self_avoid.shape[0]))
        ...
        if obstacles_avoidance is not None:
            obs = (
                obstacles_avoidance(X[:vel_start_index, N - 1])
                - self.ground_collision_safety_margin
            )
            g.append(obs)
            lbg.append(ca.DM.zeros(obs.shape[0]))
    ...
    def optimize_sequential(
        ...
        for i in range(prediction_horizon):
            ...
            if i == 0:
                ...
                start_point = X0[
                    prediction_horizon : prediction_horizon + 2 * state_dim
                ]
            ...
            if i == prediction_horizon - 1 and end_zero_velocity:
                zero_vel = True
            X_i, g_i, lbg_i, ubg_i, lbx_i, ubx_i, t_i = self.setup_single_problem(
                start_point,
                points[i + 1],
                motion_model,
                forward_kinematic,
                N=Ns[i],
                d_tol=d_tol,
                initial_point=is_initial_point,
                state_dim=state_dim,
                iteration=i,
                obstacles_avoidance=obs_avoidance,
                self_avoidance=self_avoidance,
                end_zero_velocity=zero_vel,
            )
            X = ca.vertcat(X, X_i)
            g.extend(g_i)
            lbg.extend(lbg_i)
    ...
    def generateTrajectory(
        ...
        t_bound=np.inf,
        obs_avoidance=True,
        init_config="Pontryagin",
        base_scaling_factor=0.1,
        ee_scaling_factor=0.5,
    ):
        ...
        points_full = [
            self.motion_class.end_effector_pose(starting_configuration).full().flatten()
        ]
        points_full.extend(points)
        if init_config == "Pontryagin":
            start_time = time.time()
            X0_array, ts, Ns = initial_guess(
                self.motion_class,
                points_full,
                starting_configuration,
                prediction_horizon,
                N,
                is_sequential_guess=True,
            )
            self.initialization_time = time.time() - start_time
        elif init_config == "InverseKinematics":
            start_time = time.time()
            X0_array, ts, Ns = initial_guess_simple(
                self.motion_class,
                points_full,
                starting_configuration,
                prediction_horizon,
        ...
        if obs_avoidance:
            self_avoidance = (
                self.motion_class.casadi_model_interface.signedDistanceSymMdlsPerGroup[
                    "self"
                ]
            )
            obstacles_avoidance = (
        ...
        start_time = time.time()
        X, total_elements, ts = self.optimize_sequential(
            points_full,
            prediction_horizon,
            X0,
            self.motion_model,
            ...
            Ns,
            self_avoidance=self_avoidance,
            obs_avoidance=obstacles_avoidance,
        )
    ...
    def generatePlanFromConfig(self):
        ...
        if self.load_save_generate == "load":
            ...
            X, total_elements = self.generateTrajectory(
                self.points,
                self.starting_configuration,
                self.prediction_horizon,
                N=self.N,
                t_bound=np.inf,
                init_config=self.init_config,
                obs_avoidance=self.obs_avoidance,
                base_scaling_factor=self.base_scaling_factor,
                ee_scaling_factor=self.ee_scaling_factor,
            )
            ...
            if self.slowdown_enabled:
                final_ee = (
                    self.motion_class.end_effector_pose(self.starting_configuration)
                    .full()
                    .flatten()
                )
                ...
                start_state = ca.vertcat(
                    X[-3 * self.motion_class.DoF : -2 * self.motion_class.DoF],
                    ca.DM.zeros(self.motion_class.DoF),
                    X[-self.motion_class.DoF :],
                )
                self.slowDownTrajectorySecond(
                    start_state,
                    final_ee,
                    self.motion_model,
                    self.forward_kinematic,
                    obstacle_avoidance=ground_avoidance,
                    self_avoidance=self_avoidance,
                )
            self.processResults()
            self.initiliazePartialPlanners()
            ...
            if self.slowdown_enabled:
                self.slowDownTrajectory(
                    X[-3 * self.motion_class.DoF :],
                    self.motion_class.end_effector_pose(self.starting_configuration)
                    .full()
                    .flatten(),
                    self.motion_model,
    ...
    def slowDownTrajectorySecond(
        ...
        X0_array, tf = slow_down_guess_only_ee(
            start_state, end_ee, N, self.motion_class, a_bounds=(self.u_min, self.u_max)
        )
        X0 = ca.vertcat(tf, X0_array)
        # self.slowDownTrajectory(start_state, final_ee, self.motion_model, self.forward_kinematic, obstacle_avoidance=ground_avoidance, self_avoidance=self_avoidance)
        points_full = [
            self.motion_class.end_effector_pose(start_state[: self.motion_class.DoF])
            .full()
            .flatten()
        ]
        points_full.extend([end_ee])
        state_dim = self.motion_class.DoF
        X_final, total_elements, ts = self.optimize_sequential(
            points_full,
            1,
            X0,
            self.motion_model,
            ...
            [N],
            self_avoidance=self_avoidance,
            obs_avoidance=obstacle_avoidance,
            end_zero_velocity=True,
        )
    ...
    def initiliazePartialPlanners(self):
        ...
        if self.base_point_tracking_enabled:
            self.base_planner = CasadiPartialPlanner(
                self.qs,
                self.qs_dots,
                self.us,
                self.tfs,
                self.Ns,
                base_config,
                self.motion_class.base_xytheta,
                self.motion_class.base_jacobian_xytheta,
            )
        else:
            self.base_planner = CasadiPartialPlanner(
                self.qs,
                ...
                self.Ns,
                base_config,
                self.motion_class.base_xyz,
                self.motion_class.base_jacobian,
            )

        self.ee_planner = CasadiPartialPlanner(
            self.qs,
            self.qs_dots,
            self.us,
            self.tfs,
            self.Ns,
            ee_config,
            self.motion_class.end_effector_pose,
            self.motion_class.compute_jacobian_whole,
        )
        ...
        if self.base_point_tracking_enabled:
            ...
            last_pose_base["target_pose"] = (
                self.motion_class.base_xytheta(self.qs[0]).full().flatten()
            )
            last_pose_ee["target_pos"] = (
                self.motion_class.end_effector_pose(self.qs[0]).full().flatten()
            )
            last_pose_base["initial_pose"] = (
                self.motion_class.base_xytheta(self.qs[-1]).full().flatten()
            )
            last_pose_ee["initial_pos"] = (
                self.motion_class.end_effector_pose(self.qs[-1]).full().flatten()
            )
            ...
            last_pose_ee = EEPosTrajectoryLine.getDefaultParams()
            last_pose_base["initial_pos"] = (
                self.motion_class.base_xyz(self.qs[-1]).full().flatten()
            )
            last_pose_ee["initial_pos"] = (
                self.motion_class.end_effector_pose(self.qs[-1]).full().flatten()
            )
            last_pose_base["target_pos"] = (
                self.motion_class.base_xyz(self.qs[0]).full().flatten()
            )
            last_pose_ee["target_pos"] = (
                self.motion_class.end_effector_pose(self.qs[0]).full().flatten()
            )
