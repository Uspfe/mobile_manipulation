...
class CPCPlanner(WholeBodyPlanner):
    ...
    def optimize(
        ...
        x_dyn = ca.MX.sym(
            "x_dyn", 2 * self.qs_num
        )  # x_dyn looks like [x0, x1, v0, v1], column vector
        x_dyn_num_cols = x_dyn.size()[0]
        state_dim = self.qs_num

        control = ca.MX.sym(
            "control", self.qs_num
        )  # control looks like [u0, u1], column vector
        ...
        x_sample = ca.MX.sym(
            "x_sample", total_elements
        )  # x_sample looks like [x_dyn, u0, u1, lamdas, mus, vs]

        X = ca.MX.sym(
            "X", total_elements, N
        )  # X looks like [[x_0], ... , [x_N]] where x_i = [x_i, v_i, u_i, lambda_i, mu_i, v_i]
        tn = ca.MX.sym("tn")
        dt = tn / (N)
        P = ca.MX.sym(
            "P", 2 * self.qs_num + len(points[0]) + 1
        )  # P looks like [x_0, v_0, u_0, lambda_0, mu_0, v_0]
        ...
        for i in range(1, prediction_horizon + 1):
            ...
            # print('point:', point)
            norms_array.append(
                ca.norm_2(forward_kinematic(x_dyn[:state_dim]) - ca.vertcat(*point))
                ** 2
            )

        distances = ca.Function(
            "distances", [x_dyn, vs], [ca.vertcat(*norms_array) - vs]
        )
        ...
        diff_lambdas = ca.Function(
            "diff_lambdas", [lambdas], [compute_difference_lambdas(lambdas)]
        )
        ...
        for k in range(N):  # N is the number of columns
            ...
            if k == 0:
                ...
                g.append(
                    ...
                    - dt
                    * motion_model(
                        X[:u_start_index, k - 1], X[u_start_index:u_end_index, k - 1]
                    )
                )
                lbg.append(ca.DM.zeros(x_dyn_num_cols))
                ubg.append(ca.DM.zeros(x_dyn_num_cols))
                ...
                if k == N - 1:  # last point
                    # Final lambda
                    cur_lbx[lambda_start_index:mu_start_index] = ca.DM.zeros(
                        lambda_num_cols
                    )
                    cur_ubx[lambda_start_index:mu_start_index] = ca.DM.zeros(
                        lambda_num_cols
                    )
                    # xf constraints
                ...
                # progress lambda
                g.append(
                    X[lambda_start_index:mu_start_index, k]
                    - X[lambda_start_index:mu_start_index, k - 1]
                    + X[mu_start_index:nu_start_index, k - 1]
                )
            ...
            cur_ubx[mu_start_index:nu_start_index] = ca.DM.ones(
                lambda_num_cols
            )  # bound by one as per code
            ...
            g.append(
                cpc(
                    X[mu_start_index:nu_start_index, k],
                    X[:u_start_index, k],
                    X[nu_start_index:, k],
                )
            )
            ...
            if obstacles_avoidance is not None:
                obs = (
                    obstacles_avoidance(X[:vel_start_index, k])
    ...
    def generateTrajectory(
        ...
        start_time = time.time()
        X, total_elements = self.optimize(
            points_full,
            prediction_horizon,
            X0,
            t_bound,
            ...
            N=N,
            d_tol=d_tol,
            cpc_tolerance=cpc_tolerance,
            obstacles_avoidance=obstacles_avoidance,
        )
    ...
    def generatePlanFromConfig(self):
        ...
        if self.load_save_generate == "load":
            ...
            X, total_elements = self.generateTrajectory(
                ...
                t_bound=np.inf,
                d_tol=self.d_tol,
                cpc_tolerance=self.cpc_tolerance,
                init_config=self.init_config,
                obs_avoidance=self.obs_avoidance,
            )
            if self.slowdown_enabled:
                last_state = X[
                    -3 * self.motion_class.DoF
                    - (3) * (self.prediction_horizon) : -(3) * (self.prediction_horizon)
                ]
                self.slowDownTrajectory(
                    ...
                    self.motion_model,
                    self.forward_kinematic,
                    obstacle_avoidance=obstacles_avoidance,
                )
            self.processResults()
            self.initiliazePartialPlanners()
    ...
    def initiliazePartialPlanners(self):
        if self.config is None:
            ...
            base_config = {
                "name": "PartialPlanner",
                "type": "base",
                "tracking_err_tol": self.config["tracking_err_tol"],
                "frame_id": "base",
                "ref_data_type": "Vec2",
            }
            ee_config = {
