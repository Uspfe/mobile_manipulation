...
class WholeBodyPlanner(Planner):
    ...
    def slowDownTrajectory(
        ...
        for i in range(N - 1):
            ...
            if i == 0:
                ...
                g.append(
                    X[:u_start_index, i]
                    - start_state[:u_start_index]
                    - dt
                    * motion_model(
                        start_state[:u_start_index], start_state[u_start_index:]
                    )
                )
                lbg.append(ca.DM.zeros(2 * state_dim))
                ubg.append(ca.DM.zeros(2 * state_dim))
        ...
        if obstacle_avoidance is not None:
            obs = (
                obstacle_avoidance(X[:vel_start_index, N - 1])
        ...
        points_full = [
            self.motion_class.end_effector_pose(start_config).full().flatten()
        ]
        ...
        res = solver(
            x0=X0,
            lbx=ca.vertcat(0, *lbx),
            ubx=ca.vertcat(np.inf, *ubx),
            lbg=ca.vertcat(*lbg),
            ubg=ca.vertcat(*ubg),
        )
        X_final = res["x"]
        ...
        compare_trajectories_casadi_plot(
            ...
            q_size=state_dim,
            show=True,
            a_bounds=[(self.u_min, self.u_max), (self.u_min, self.u_max)],
            v_bounds=(self.q_dot_min, self.q_dot_max),
        )
        plot_obstacle_avoidance(
            [X_final, X0],
            self_avoidance,
            ...
            q_size=state_dim,
            show=True,
            labels=["Slow Down", "Initial guess"],
            limit=self.self_collision_safety_margin,
        )
    ...
    def processResults(self):
        ...
        qs_zero, qs_dot_zero, us_zero = decompose_X(
            self.X0, self.qs_num, self.total_elements
        )
        self.qs, self.qs_dots, self.us = qs.T, qs_dots.T, us.T
        self.qs_zero, self.qs_dot_zero, self.us_zero = (
            qs_zero.T,
            qs_dot_zero.T,
            us_zero.T,
        )
        if self.slowdown_enabled and self.X_slow is not None:
            ...
            qs_slow, qs_dots_slow, us_slow = decompose_X(
                self.X_slow, self.qs_num, total_elements_slow
            )
            self.qs_slow, self.qs_dots_slow, self.us_slow = (
                qs_slow.T,
                qs_dots_slow.T,
                us_slow.T,
            )
            self.tfs = np.concatenate(
                (self.tfs, np.array([float(self.X_slow[0])])), axis=0
            )
    ...
    def interpolate(self, t):
        ...
        q = self.qs[lower_index] + (self.qs[upper_index] - self.qs[lower_index]) * (
            t - lower_time
        ) / (upper_time - lower_time)
        q_dot = self.qs_dots[lower_index] + (
            self.qs_dots[upper_index] - self.qs_dots[lower_index]
        ) * (t - lower_time) / (upper_time - lower_time)
        q_dot_dot = self.us[lower_index] + (
            self.us[upper_index] - self.us[lower_index]
        ) * (t - lower_time) / (upper_time - lower_time)
...
class CasadiPartialPlanner(Planner):
    def __init__(
        ...
        Ns,
        config,
        pose_calculator_func,
        jacobian_calculator_func,
    ):
        ...
        for i in range(len(self.qs)):
            self.plan["p"].append(
                self.pose_calculator_func(self.qs[i]).full().flatten()
            )
