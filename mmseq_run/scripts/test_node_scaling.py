...
def sample_3d_points(points_len, perimeter_size, z_max=2):
    min_val = (
        -perimeter_size
    ) * 2  # double so points can be 0.5 apart after diving by two
...
def main():
    ...
    parser.add_argument(
        "--optimization", required=False, help="Optimization type", default="cpc"
    )
    parser.add_argument(
        "--scenarios", required=False, help="Loop trajectory", default=1
    )
    parser.add_argument("--max_N", required=False, help="Max N", default=100)
    parser.add_argument(
        "--obstacle_avoid", action="store_true", help="Obstacle avoidance"
    )
    ...
    # setup small example
    list_of_Ns = np.linspace(
        100, int(args.max_N), int(int(args.max_N) / 100), dtype=int
    )
    print(list_of_Ns)
    pi = 3.14
    ...
    v_bounds = (
        mobile_robot.lb_x[mobile_robot.DoF :],
        mobile_robot.ub_x[mobile_robot.DoF :],
    )
    ...
    for i in range(int(args.scenarios)):
        ...
        if label == "ee_test":
            ...
            first_point = sample_3d_points(
                1, perimeter_size, z_max=0
            )  # just so that the points are not always around the origin
            points += first_point
        ...
        points_full = [
            mobile_robot.end_effector_pose(starting_configuration).full().flatten()
        ]
        ...
        for N in list_of_Ns:
            ...
            results, dict_res, shortest = mobile_robot.calculate_trajectory(
                points_full,
                starting_configuration,
                prediction_horizon,
                vertex_angle_deg=40,
            ...
            for j in init_configs:
                ...
                if args.optimization == "cpc":
                    if j == "inverse_kinematic":
                        start_time = time.time()
                        X0_array, tf = initial_guess_simple(
                            mobile_robot,
                            points_full,
                            starting_configuration,
                            prediction_horizon,
                        ...
                        print(
                            f"Time taken to compute initial guess inverse kynematic for scenario {i}: {initial_guess_time}"
                        )

                    elif j == "pontryagin":
                        start_time = time.time()
                        X0_array, tf = initial_guess(
                            mobile_robot,
                            points_full,
                            starting_configuration,
                            prediction_horizon,
                        ...
                        initial_guess_time = time.time() - start_time
                        print(
                            f"Time taken to compute initial guess Pontryagin for scenario {i}: {initial_guess_time}"
                        )

                    X0 = ca.vertcat(tf, *X0_array)
                    start_time = time.time()
                    X, total_elements = cpc_plan.optimize(
                        points_full,
                        prediction_horizon,
                        X0,
                        np.inf,
                        ...
                        forward_kinematic,
                        N=N,
                        obstacles_avoidance=obstacles_avoidance_for_opti,
                    )
                ...
                ## Sequential optimization
                    ...
                    if j == "inverse_kinematic":
                        start_time = time.time()
                        X0_array, ts_guess, Ns = initial_guess_simple(
                            mobile_robot,
                            points_full,
                            starting_configuration,
                            prediction_horizon,
                        ...
                        start_time = time.time()
                        X0_array, ts_guess, Ns = initial_guess(
                            mobile_robot,
                            points_full,
                            starting_configuration,
                            prediction_horizon,
                    ...
                    start_time = time.time()
                    X, total_elements, ts = sequential_plan.optimize_sequential(
                        points_full,
                        prediction_horizon,
                        X0,
                        motion_model,
                    ...
                    optimization_time = time.time() - start_time
                    print(
                        f"Time taken from Sequential scenario {i}: {optimization_time}"
                    )
                    print(f"End time scenario {i}: {X[0]}")
                    print()
                ...
                # data collection
                experiment_data["Scenario " + str(i)][j]["initial_guess_ts"].append(
                    initial_guess_time
                )
                experiment_data["Scenario " + str(i)][j]["optimization_ts"].append(
                    optimization_time
                )
                experiment_data["Scenario " + str(i)][j]["dt_std"].append(np.std(dts))
                experiment_data["Scenario " + str(i)][j]["total_elements"].append(
                    total_elements
                )
        ...
        for j in init_configs:
            ...
            fig = plot_3d_trajectory(
                ...
                show=False,
                title=f"Initial guess {j} scenario {i}",
            )
            ...
            fig = plot_3d_trajectory(
                ...
                labels=plotting_labels,
                show=False,
                title=f"Final solution {j} scenario {i}",
            )
            pdf.savefig(fig)
            plt.close(fig)
            # plot velocities
            fig = compare_trajectories_casadi_plot(
                experiment_data["Scenario " + str(i)][j]["final_solutions"],
                points_full,
                None,
                None,
            ...
            tfs = [
                float(X[0])
                for X in experiment_data["Scenario " + str(i)][j]["final_solutions"]
            ]
            string = "Final times ->"
            ...
            fig, obstacles_violated = plot_obstacle_avoidance(
                experiment_data["Scenario " + str(i)][j]["final_solutions"],
                obstacles_avoidance,
                experiment_data["Scenario " + str(i)][j]["total_elements"],
                show=False,
            ...
            fig = simple_plot(
                ...
                y_label="Time",
                x_label="N",
                titles=[f"Initialization time for Scenario {i}"],
                show=False,
            )
            pdf.savefig(fig)
            plt.close(fig)
            ...
            fig = simple_plot(
                ...
                y_label="Time",
                x_label="N",
                titles=[f"Optimization time for Scenario {i}"],
                show=False,
            )
            pdf.savefig(fig)
            plt.close(fig)
            ...
            fig = simple_plot(
                ...
                [
                    [
                        [
                            float(X[0])
                            for X in experiment_data["Scenario " + str(i)][j][
                                "final_solutions"
                            ]
                        ]
                    ]
                ...
                x_label="N",
                show=False,
                titles=[f"Final time for Scenario {i}"],
            )
            pdf.savefig(fig)
            plt.close(fig)
    ...
    for init_method in init_configs:
        ...
        for i in range(int(args.scenarios)):
            init_times.append(
                experiment_data["Scenario " + str(i)][init_method]["initial_guess_ts"]
            )
            opti_times.append(
                experiment_data["Scenario " + str(i)][init_method]["optimization_ts"]
            )
            labels.append(f"Scenario {i}")
            final_times.append(
                [
                    float(X[0])
                    for X in experiment_data["Scenario " + str(i)][init_method][
                        "final_solutions"
                    ]
                ]
            )
    ...
    fig = simple_plot(
        ...
        labels=labels,
        titles=[
            f"Initial guess time Pontryagin {args.optimization}",
            f"Initial guess time Inverse kinematic {args.optimization}",
        ],
    )
    ...
    fig = simple_plot(
        ...
        labels=labels,
        titles=[
            f"Optimization time Pontryagin {args.optimization}",
            f"Optimization time Inverse kinematic {args.optimization}",
        ],
    )
    pdf.savefig(fig)
    plt.close(fig)
    ...
    fig = simple_plot(
        ...
        labels=labels,
        titles=[
            f"Final time Pontryagin {args.optimization}",
            f"Final time Inverse kinematic {args.optimization}",
        ],
    )
    pdf.savefig(fig)
    plt.close(fig)
    ...
    for init_method in init_configs:
        ...
        for i in range(int(args.scenarios)):
            dt_std.append(
                experiment_data["Scenario " + str(i)][init_method]["dt_std"]
            )  # will contain a list of stds (as many as the number of Ns) per scenario
        dt_std = np.array(dt_std).T.tolist()
        data.append(dt_std)
    fig = plot_whisker_plots(
        data,
        plotting_labels,
        show=False,
        titles=[
            "Standard dev of dts with Pontryagin",
            "Standard dev of dts with Inverse kinematic",
        ],
        ylabel="Standard deviation of dts",
    )
    pdf.savefig(fig)
    plt.close(fig)
