...
def main():
    ...
    parser.add_argument(
        "--scenarios", required=False, help="Loop trajectory", default=10
    )
    parser.add_argument(
    ...
    for i in range(int(args.scenarios)):
        ...
        fig = plot_3d_trajectory(
            ...
            title=f"Initial Guess Scenario {i} {label}",
            show=False,
        )
        fig.text(
            0.5,
            0.1,
            f'inverse kinematic computation time: {timing_data["inverse_kinematic"][label]["initial_guess_ts"][-1]}, pontryagin computation time: {timing_data["pontryagin"][label]["initial_guess_ts"][-1]}',
            ha="center",
        )
        ...
        fig = plot_3d_trajectory(
            ...
            q_size=qs_num,
            labels=plotting_labels,
            title=f"Final Solution Scenario {i} {label}",
            show=False,
        )
        fig.text(
            0.5,
            0.1,
            f'inverse kinematic computation time: {timing_data["inverse_kinematic"][label]["optimization_ts"][-1]}, pontryagin computation time: {timing_data["pontryagin"][label]["optimization_ts"][-1]}',
            ha="center",
        )
        pdf.savefig(fig)
        plt.close(fig)
    ...
    fig = plot_whisker_plots(
        [initialization_data, optimization_data],
        x_axis,
        show=False,
        titles=[
            f"Initialization {args.optimization}",
    ...
    for i in range(len(scenario_labels)):
        ...
        fig = simple_plot(
            np.arange(len(final_times[0])),
            [final_times],
            show=False,
            y_label="Time [s]",
            x_label="Scenarios",
            labels=init_configs,
            titles=[f"Final Times {args.optimization} {scenario_labels[i]}"],
        )
        pdf.savefig(fig)
        plt.close(fig)
    # overall figgure
    ...
    fig = plot_whisker_plots(
        ...
        x_axis,
        show=False,
        titles=["Standard Deviation of dts"],
        ylabel="Standard Deviation",
    )
    fig.text(
        0.5,
        0.01,
        f"Scenarios violating obstacles: {scenarios_violating_obstacles}",
        ha="center",
    )
    pdf.savefig(fig)
    plt.close(fig)
    ...
    fig = simple_plot(
        ...
        x_label="Scenarios",
        labels=init_configs,
        titles=[f"Final Times {args.optimization}"],
    )
    pdf.savefig(fig)
    plt.close(fig)
    pdf.close()
