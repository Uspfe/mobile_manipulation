...
def main():
    ...
    parser.add_argument(
        "--init", required=False, help="Initial guess type", default="pont"
    )
    ...
    if args.optimization == "cpc":
        ...
        for i in range(int(args.loop)):
            if args.init == "inverse":
                ...
                print(
                    f"Time taken to compute initial guess inverse kynematic: {time.time()-start_time}"
                )
                ...
                print(
                    f"Time taken to compute initial guess Pontryagin: {time.time()-start_time}"
                )
    ...
    ## Sequential optimization
        ...
        for i in range(int(args.loop)):
            ...
            start_time = time.time()
            X, total_elements, _ = sequential_plan.optimize_sequential(
                points_full, prediction_horizon, X0, motion_model, forward_kinematic, Ns
            )
            t.append(time.time() - start_time)
