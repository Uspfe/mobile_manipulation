...
if __name__ == "__main__":
    ...
    controller_data = {
        name: data_plotter.data.pop("mpc_" + name + "s") for name in ctrl_mode_names
    }
    ...
    for name in ctrl_mode_names:
        ...
        for key in data_plotter.data.keys():
            if (
                isinstance(data_plotter.data[key], np.ndarray)
                and data_plotter.data[key].shape != ()
            ):
        ...
        data_ctrl = [
            data_plotter._convert_np_array_to_dict(data_ctrl[t_index])
