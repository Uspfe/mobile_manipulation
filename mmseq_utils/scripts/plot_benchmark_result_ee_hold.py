...
BASE_WAYPOINTS = np.array([[0, 0, 0], [2.5, -0.0, 0.0], [7, 1.0, 0.0]])
...
class EEHoldDataPlotter(ROSBagPlotter):
    ...
    def _trim_data(self):
        ...
        time_start_index_base = np.where(np.linalg.norm(r_b[:, :2], axis=-1) < 0.01)[0][
            0
        ]
        time_end_index_base = np.where(
            self.data["ridgeback"]["joint_states"]["ts"] < TASK_CHANGE_TIME
        )[0][-1]
        ...
        for key, data in self.data["ridgeback"]["joint_states"].items():
            self.data["ridgeback"]["joint_states"][key] = data[
                time_start_index_base:time_end_index_base
            ]

        # trime ee
        time_start_index_ee = np.where(
            self.data["model"]["EE"]["ts"] - time_start < 0.01
        )[0][-1]
        time_end_index_ee = np.where(self.data["model"]["EE"]["ts"] < TASK_CHANGE_TIME)[
            0
        ][-1]
...
class EEHoldBenchmarkPlotter:
    ...
    def plot_base_and_ee_path(self):
        ...
        for id, p in enumerate(self.plotters):
            axes[0] = p.plot_base_path(
                axes[0], id, worldframe=False, linewidth=line_width[id]
            )
        axes[0].plot(
            BASE_WAYPOINTS[:, 0],
            BASE_WAYPOINTS[:, 1],
        ...
        for id, p in enumerate(self.plotters):
            axes[1] = p.plot_ee_path(
                axes[1], id, worldframe=False, linewidth=line_width[id]
            )
        axes[1].plot(
            EE_WAYPOINT[0],
            EE_WAYPOINT[1],
        ...
        for id, x in enumerate(xs):
            ...
            axes[id].axvline(
                x, color="darkgreen", linestyle="--"
            )  # Optional visual for x-value

        plt.subplots_adjust(
            top=0.952, bottom=0.12, left=0.152, right=0.967, hspace=0.045, wspace=0.2
        )
