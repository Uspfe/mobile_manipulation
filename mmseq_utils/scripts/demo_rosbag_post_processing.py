...
def add_human_visualization(swarm_state_msg):
    ...
    for i, pose in enumerate(swarm_state_msg.poses):
        marker = make_marker(Marker.CYLINDER, i, RBY_COLORS[i], [0.2, 0.2, pose.position.z], lifetime=0.1)
        marker.pose.position = Point(pose.position.x, pose.position.y, pose.position.z/2)
