...
from mmseq_utils.plotting import (
    DataPlotter,
    multipage,
    construct_logger,
    HTMPCDataPlotter,
...
def benchmark(folder_path):
    ...
    if "stats_all.csv" in os.listdir(folder_path):
        df = pandas.read_csv(
            os.path.join(folder_path, "stats_all.csv"), index_col=[0, 1]
        )
        print(df)
    else:
        dfs = []
    ...
    df_summary = concat(
        [df_mpc.mean(), df_idkc.mean()], keys=["HTMPC", "HTIDKC"], axis=1
    )
    df_summary.to_csv(os.path.join(folder_path, "stats_summary.csv"))

    diff_ee = (
        (df_mpc["err_ee_integral"] - df_idkc["err_ee_integral"])
        / df_idkc["err_ee_integral"]
        * 100
    )
    diff_base = (
        (
            df_mpc["err_base_normalized_integral"]
            - df_idkc["err_base_normalized_integral"]
        )
        / df_idkc["err_base_normalized_integral"]
        * 100
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sdf", action="store_true", help="plot sdf")
    ...
    parser.add_argument("--sdfdebug", action="store_true", help="debug")
    parser.add_argument("--mpcdebug", action="store_true", help="debug")
    parser.add_argument("--compare", action="store_true", help="plot comparisons")
    parser.add_argument(
        "--htmpc", action="store_true", default=False, help="use htmpc plotter"
    )
    parser.add_argument("--compareidkc", action="store_true", help="plot comparisons")
    parser.add_argument(
        "--benchmark", action="store_true", help="gather benchmark results"
    )
    parser.add_argument(
        "--savefigs", action="store_true", help="export figures to file"
    )
    parser.add_argument(
        "--printparam", action="store_true", help="export figures to file"
    )
    parser.add_argument("--paramname", type=str, help="control timestep to inspect")
    parser.add_argument(
        "--plottimeoptimal", action="store_true", help="export figures to file"
    )
    ...
    if args.compareidkc:
        ...
        data_plotter = construct_logger(
            args.folder, is_htmpc=args.htmpc, data_plotter_class=plotter_class
        )
        if args.control:
