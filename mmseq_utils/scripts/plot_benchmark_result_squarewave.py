...
STATS = [
    ...
    ("err_ee", "max"),
    ("err_ee_1", "integral"),
    ("err_base_normalized_1", "integral"),
    ("err_ee_2", "integral"),
    ("err_base_normalized_2", "integral"),
    ("cmd_accs_saturation", "mean"),
    ("cmd_jerks_base_linear", "max"),
    ("run_time", "mean"),
    ("constraints_violation", "mean"),
    ("constraints_violation", "num"),
    ("phi", "value"),
    ("r", "value"),
]
YELLOW = "#FFAE42"
...
def get_optimal_base_err(r_bw_wd, r_ew_wd):
    r_bw_w_opt = (r_bw_wd - r_ew_wd[:2]) / np.linalg.norm(
        r_bw_wd - r_ew_wd[:2]
    ) * REACHABLE_RADIUS + r_ew_wd[:2]
...
def construct_logger(path_to_folder):
    ...
    if folder_num == 2:
        ...
        return SquareWaveDataPlotter.from_PYSIM_results(
            os.path.join(path_to_folder, items[0])
        )
...
class SquareWaveDataPlotter(DataPlotter):
    ...
    def _get_statistics_squarewave(self):
        N = np.argwhere(
            np.linalg.norm(self.data["r_bw_w_ds"] - self.data["r_bw_w_ds"][-10], axis=1)
            < 1e-3
        ).flatten()[0]
        ...
        eb_opt = get_optimal_base_err(
            self.data["r_bw_w_ds"][0], self.data["r_ew_w_ds"][-10]
        )
        # eb_opt = self.data["err_base"][N-10]

        self.data["err_base_normalized_1"] = (self.data["err_base"][:N]) / (
            self.data["err_base"][0]
        )
        self.data["err_base_normalized_2"] = (
            self.data["err_base"][N:] / self.data["err_base"][N]
        )

        data_names = [
            "err_ee_1",
            "err_ee_2",
            "err_base_normalized_1",
            "err_base_normalized_2",
        ]
        t1 = self.data["ts"][:N]
        t2 = self.data["ts"][N:]
        for name in data_names:
            ...
            self.data["statistics"][name] = {
                "rms": math.rms_continuous(t, np.abs(self.data[name])),
        ...
        self.data["statistics"]["phi"] = {
            "value": math.wrap_to_2_pi_scalar(np.arctan2(r_be_w[1], r_be_w[0]))
        }
    ...
    def plot_mpc_cost(self, axes, index=0, legend=None, titles=None):
        ...
        for l in range(task_num):
            if l != 1 or not task_1_failed:
                axes[l].plot(
                    t_sim, cost[:, l], label=legend, color=colors[index], linewidth=1.3
                )
            else:
                axes[l].plot(
                    t_sim[: half_time_index - 20],
                    cost[: half_time_index - 20, l],
                    label=legend,
                    color=colors[index],
                    linewidth=1.3,
                )
    ...
    def plot_task_performance(self, axes=None, index=0, legend=None):
        ...
        axes[2].plot(
            t_sim[:N],
            self.data["err_base_normalized_1"],
            label=legend
            + " part 1 acc = {:.3f}".format(
                self.data["statistics"]["err_base_normalized_1"]["integral"]
            ),
            color=colors[index],
        )
        axes[2].plot(
            ...
            linestyle="--",
            label=legend
            + " part 2 acc = {:.3f}".format(
                self.data["statistics"]["err_base_normalized_2"]["integral"]
            ),
            color=colors[index],
        )
    ...
    def plot_optimal_error(self, axes=None):
        ...
        err_opt_1 = get_optimal_base_err(
            self.data["r_bw_w_ds"][0], self.data["r_ew_w_ds"][0]
        )
        err_opt_2 = 0
        ...
        t_sim = self.data["ts"]
        axes.plot(
            t_sim, err_opt, "--", color=SCARLET, linewidth=1.5, label="optimal error"
        )
        axes.legend(fontsize=FOOTNOTE_SIZE)
    ...
    def plot_base_tracking_normalized(self, axes=None, index=0, legend=None):
        ...
        axes.plot(
            t_sim,
            np.hstack(
                (self.data["err_base_normalized_1"], self.data["err_base_normalized_2"])
            ),
            color=colors[index],
            label=legend,
            linewidth=1.3,
...
class BenchmarkDataPlotter:
    ...
    def summarize(self, methods):
        ...
        df_summary.to_csv(
            os.path.join(
                self.folder_path, "stats_summary_{}.csv".format("_".join(methods))
            )
        )
    ...
    def plot(self, dfs, names):
        diff_ee = (
            (dfs[0]["err_ee_integral"] - dfs[1]["err_ee_integral"])
            / dfs[1]["err_ee_integral"]
            * 100
        )
        diff_base = (
            (dfs[0]["err_base_integral"] - dfs[1]["err_base_integral"])
            / dfs[1]["err_base_integral"]
            * 100
        )
    ...
    def plot_mean_error(self, dfs, names):
        ...
        for i in range(num_dfs):
            axes1[0].hist(
                dfs[i]["err_ee_1_integral"], bins=100, label=names[i], alpha=0.5
            )
            axes1[1].hist(
                dfs[i]["err_ee_2_integral"], bins=100, label=names[i], alpha=0.5
            )
        ...
        for i in range(num_dfs):
            axes2[0].hist(
                dfs[i]["err_base_normalized_1_integral"],
                bins=100,
                label=names[i],
                alpha=0.5,
            )
            axes2[1].hist(
                dfs[i]["err_base_normalized_2_integral"],
                bins=100,
                label=names[i],
                alpha=0.5,
            )
    ...
    def violin_plot(self, dfs, df_names, x_labels=None, axes=None, base_only=False):
        ...
        df_ee = self._convert_header_to_label(
            df,
            ["err_ee_1_integral", "err_ee_2_integral"],
            ["part 1", "part 2"],
            ["Square Wave"],
            "Mean Error (m)",
        )
        df_base = self._convert_header_to_label(
            df,
            ["err_base_normalized_1_integral", "err_base_normalized_2_integral"],
            ["part 1", "part 2"],
            ["Square Wave"],
        ...
        df_ee.to_csv(
            os.path.join(
                self.folder_path, "err_ee_{}_stacked.csv".format("_".join(df_names))
            )
        )
        df_base.to_csv(
            os.path.join(
                self.folder_path, "err_base_{}_stacked.csv".format("_".join(df_names))
            )
        )
        with sns.color_palette("ch:s=.35,rot=-.35", n_colors=3):
            if axes is None:
                if not base_only:
                    ...
                    fig, axes_base = plt.subplots(figsize=(3.5, 2.0))
                    axes = [axes_base]
            legend_handles = []
            if not base_only:
                sns.stripplot(
        ...
        if not base_only:
            plt.subplots_adjust(
                top=0.975, bottom=0.095, left=0.125, right=0.9, hspace=0.24, wspace=0.2
            )
        else:
            plt.subplots_adjust(
                top=0.98, bottom=0.18, left=0.145, right=0.96, hspace=0.24, wspace=0.2
            )
        return axes, fig
    ...
    def radial_heat_map(
        self, dfs, df_names, x_var_name, y_var_name, title=None, ax=None
    ):
        # df = concat(dfs, keys=df_names, names=["Method"])
        ...
        cm = sns.diverging_palette(
            140, 15, s=65, l=45, sep=1, n=50, center="light", as_cmap=True
        )

        if ax is None:
            fig, ax = plt.subplots(
                subplot_kw={"projection": "polar"}, figsize=(3.5, 2.5)
            )
        else:
            fig = plt.gca()
        ...
        # Annotation
        impacted_area = mpatches.Ellipse(
            (0.0, radius[-1] / 2),
            height=0.5,
            width=2.5,
            angle=-90,
        ...
        ax.set_rmax(2.5)
        ax.set_xticks((np.arange(12) + 1) / 6.0 * np.pi)
        ...
        plt.subplots_adjust(
            top=1.0, bottom=0.0, left=0.15, right=1.0, hspace=0.2, wspace=0.2
        )

        return ax, fig

    def radial_heat_map_seperate(
        self, dfs, df_names, x_var_name, y_var_name, title=None, ax=None
    ):
        # df = concat(dfs, keys=df_names, names=["Method"])
        # df.rename(columns={y_var_name: "values"}, inplace=True)
        ...
        if ax is None:
            fig, axes = plt.subplots(
                1, df_num, subplot_kw={"projection": "polar"}, figsize=(3.5, 1.5)
            )
        else:
            fig = plt.gca()
        # Defining the colormesh to define the color as per the values
    ...
    def add_arrow_to_line2D(
        ...
        arrow_kw = {"arrowstyle": arrowstyle, "mutation_scale": 10 * arrowsize}
    ...
    def plot_squarewave_tasks(self, df, ax=None, scale=1):
        ...
        ax.annotate(
            "Workspace\nBoundary",
            xy=(np.pi / 2, REACHABLE_RADIUS),
            xycoords="data",
            xytext=(np.pi / 2, REACHABLE_RADIUS * 0.75),
            horizontalalignment="center",
            fontsize=FOOTNOTE_SIZE * scale,
        )
        ...
        N = np.argwhere(
            np.linalg.norm(r_bw_b_ds - r_bw_b_ds[-10], axis=1) < 1e-3
        ).flatten()[0]
        ...
        ax.scatter(
            df.xs("test_13")["phi_value"],
            df.xs("test_13")["r_value"],
            facecolors=YELLOW,
            edgecolors=YELLOW,
            linewidth=1.5,
            s=10,
        ...
        phi_valley = math.wrap_to_2_pi_scalar(
            np.arctan2(base_target_valley[1], base_target_valley[0])
        )
        r_valley = np.linalg.norm(base_target_valley)
        ax.scatter(phi_valley, r_valley, facecolors=YELLOW, s=10)
        ax.annotate(
            "Base Home/\nTarget (Valley)",
            xy=(phi_valley, r_valley),
            xycoords="data",
            xytext=(np.pi, r_valley * 0.7),
            horizontalalignment="center",
            fontsize=FOOTNOTE_SIZE * scale,
        )
        ...
        ax.set_xticks(
            math.wrap_to_2_pi_array((np.arange(12) + 1) / 6.0 * np.pi + mid - np.pi / 2)
        )
        ...
        plt.subplots_adjust(top=0.929, bottom=5.2e-18, hspace=0.2, wspace=0.2)
...
def benchmark(folder_path):
    ...
    methods = [
        "Baseline - High Tolerance",
        "Baseline - No Tolerance",
        "Our - High Tolerance",
        "Our - No Tolerance",
    ]
    # names = ["Baseline\nLexConstraint\n(No_Tol)", "Baseline\nLexConstraint\n(with_Tol)", "Proposed\nLexConstraint\n(with_Tol)"]
    ...
    data_plotter.box_plot(
        "cmd_jerks_base_linear_max",
        dfs,
        methods,
        "Maximum Commanded Base Jerk (Linear) (m/s^3)",
    )
...
def ral_paper(folder_path):
    ...
    _, fig_singularity = data_plotter.radial_heat_map(
        dfs,
        methods,
        "phi_value",
        "err_base_normalized_integral",
...
def tracking(folder_path):
    ...
    for pid, plotter in enumerate(plotters):
        ...
        shade_region.append(
            [plotter.data["ts"][index][0], plotter.data["ts"][index][-1]]
        )
    ...
    axes[0].fill_between(
        np.linspace(shade_start, shade_end, 100),
        -0.2,
        2.2,
        facecolor=SCARLET,
        alpha=0.15,
    ...
    an12 = axes[0].annotate(
        "",
        xy=(0.0, -axes_fraction_y),
        xycoords=("data", "axes fraction"),
        xytext=(8, -axes_fraction_y),
        arrowprops=dict(arrowstyle="<->", lw=1.0),
    )
    an11 = axes[0].annotate(
        "Part 1 (Peak)",
        xy=(4, -axes_fraction_y),
        xycoords=("data", "axes fraction"),
        va="center",
        ha="center",
    ...
    an22 = axes[0].annotate(
        "",
        xy=(8, -axes_fraction_y),
        xycoords=("data", "axes fraction"),
        xytext=(16.0, -axes_fraction_y),
        arrowprops=dict(arrowstyle="<->", lw=1.0),
    )

    an21 = axes[0].annotate(
        "Part 2 (Valley)",
        xy=(12, -axes_fraction_y),
        xycoords=("data", "axes fraction"),
        va="center",
        ha="center",
        bbox=dict(boxstyle="round", fc="w"),
    ...
    an_sing_text = axes[1].annotate(
        "Singular\nConfiguration",
        xy=(np.mean([shade_start, shade_end]), 0.5),
        xycoords="data",
        va="center",
        ha="center",
        bbox=dict(boxstyle="round", fc="w"),
    ...
    plt.subplots_adjust(
        top=0.97, bottom=0.168, left=0.135, right=0.977, hspace=0.222, wspace=0.0
    )
...
def cost(folder_path):
    ...
    axes = None
    legends = [
        "HTMPC_Baseline_Constraint_No_Tol",
        "HTMPC_Baseline_Constraints_New",
        "HTMPC",
    ]
    for pid, plotter in enumerate(plotters):
        axes, fig = plotter.plot_mpc_cost(
            axes=axes,
            index=pid,
            titles=["Task 1: EE Tracking Cost", "Task 2: Base Tracking Cost"],
        )
    for ax in axes:
        ax.grid("on")
    plt.subplots_adjust(
        top=0.92, bottom=0.135, left=0.1, right=0.955, hspace=0.245, wspace=0.2
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tracking", action="store_true", help="plot tracking comparisons"
    )
    parser.add_argument("--cost", action="store_true", help="plot tracking comparisons")
    parser.add_argument("--ral", action="store_true", help="plot tracking comparisons")
