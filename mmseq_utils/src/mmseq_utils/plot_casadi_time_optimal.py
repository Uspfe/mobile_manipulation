...
from mmseq_utils.point_mass_computation_scripts.point_mass_trajectory_optimization import (
    space_curve,
    velocity_curve,
    acceleration_curve,
)
...
def compare_trajectories_casadi_plot(
    casadi_results,
    points,
    dict_res,
    shortest,
    ...
    for obstacle in obstacles:
        circle = plt.Circle(
            (obstacle.x, obstacle.y), obstacle.radius, color="grey", alpha=0.5
        )
    ...
    if dict_res != None:
        ...
        for i in range(len(shortest) - 1):
            ...
            for j in range(q_size):
                ...
                q_dots_section = [
                    velocity_curve(
                        t, par[1], par[2], par[3], par[4], par[5], par[7], par[8]
                    )
                    for t in t_values
                ]
                us_section = [
                    acceleration_curve(t, par[2], par[5], par[7], par[8])
                    for t in t_values
                ]
                if i == 0:
...
def plot_3d_trajectory(
    ...
    if dict_res != None:
        ...
        if base_kin:
            ...
            ax.plot(
                ...
                ys,
                zs,
                label="Fastest base trajectory, sampling algo",
                color="red",
                linestyle="--",
            )
    # plot points
    ...
    # plot start point
    ax.scatter(
        points[0][0], points[0][1], points[0][2], color="purple", label="Start point"
    )
    ax.legend()
    # add title
    ax.set_title(title)
...
def plot_obstacle_avoidance(
    ...
    number_of_plots = max(
        obstacles_avoidance(np.zeros(q_size)).shape[0],
        obstacles_avoidance(np.zeros(q_size)).shape[1],
    )
    # define the subplots
    rows = int(np.ceil(np.sqrt(number_of_plots)))
    cols = int(np.ceil(number_of_plots / rows))
...
def plot_base_ee_velocities(
    casadi_results, motion_class, state_dim, ts, Ns, q_size=2, labels=[], show=True
):
    # define the subplots
    rows = 3
    cols = 2
    ...
    velocities_titles = [
        ...
        "End effector velocity y",
        "End effector velocity z",
    ]
    ts = [i.full().flatten() for i in ts]
...
def simple_plot(
    x, Y, titles=["Simple plot"], x_label="x", y_label="y", show=True, labels=[]
):
...
def plot_motion_model(
    casadi_results, motion_model, state_dim, q_size=2, labels=[], show=True, limit=0
):
