...
from .point_mass_trajectory_optimization import (
    space_curve,
    velocity_curve,
    acceleration_curve,
)
...
def initial_guess(
    ...
    is_sequential_guess=False,
):
    results, dict_res, shortest = mobile_robot.calculate_trajectory(
        points, starting_configurations, prediction_horizon, magnitude_step=1
    )
    ...
    for i in range(len(shortest) - 1):
        ...
        for k in range(Ns[i]):
            if k == 0 and i == 0:
                X0_no_tn.extend(
                    [*qs_reshaped[:, k], *q_dots_reshaped[:, k], *us_reshaped[:, k]]
                )
            else:
                X0_no_tn.extend(
                    [
                        *qs_reshaped[:, k]
                        + position_noise * np.random.random(qs_reshaped[:, k].shape),
                        *q_dots_reshaped[:, k]
                        + velocity_noise
                        * np.random.random(q_dots_reshaped[:, k].shape),
                        *us_reshaped[:, k]
                        + acceleration_noise
                        * np.random.random(us_reshaped[:, k].shape),
                    ]
                )
            ...
            if not is_sequential_guess:
                ...
                for j in range(1, prediction_horizon + 1):
                    ...
                    norm = np.linalg.norm(
                        end_effector_pose_func([*qs_reshaped[:, k]]) - p
                    )
                    if norm <= d_tol:
                        mus_array.append(1)
...
def equally_spaced_in_time(i, shortest, dict_res, N=100):
    ...
    if i == 0:
        ...
        t_values = np.linspace(
            0, params[0][6], N + 1
        )  # this way, there will not be two overlapping points
        t_values = t_values[1:]
    ...
    for i in range(len(params)):
        ...
        us.append(
            [
                acceleration_curve(
                    t, params[i][2], params[i][5], params[i][7], params[i][8]
                )
                for t in t_values
            ]
        )
...
def initial_guess_simple(
    ...
    waypoints_poses, intermediate_points = mobile_robot.calculate_trajectory_simple(
        points, starting_configurations, prediction_horizon
    )
    ...
    for i in range(prediction_horizon):
        ...
        interpolated_qs = np.array(
            [np.interp(interpolation_times, controller_times, q) for q in qs]
        ).T

        interpolated_q_dots = np.array(
            [
                np.interp(interpolation_times, controller_times, q_dot)
                for q_dot in q_dots
            ]
        ).T
        for k in range(n):
            ...
            if not is_sequential_guess:
                ...
                for j in range(1, prediction_horizon + 1):
                    ...
                    p = np.array([point[0], point[1], point[2]])
                    norm = np.linalg.norm(
                        end_effector_pose_func(interpolated_qs[k]) - p
                    )
                    if norm <= d_tol:
                        mus_array.append(1)
                        norms_array.append(norm**2)
...
def slow_down_guess(
    ...
    velocities[1] = {}
    velocities[0][0] = [
        start_state[mobile_robot.DoF : 2 * mobile_robot.DoF].full().flatten()
    ]
    velocities[1][0] = [
        end_state[mobile_robot.DoF : 2 * mobile_robot.DoF].full().flatten()
    ]
    ...
    results, dict_res, shortest = compute_all_trajectories_multi_pose(
        dict_graph, velocities, a_max, a_min, v_max, v_min, prediction_horizon=1
    )
            ...
            X0_no_tn.extend(
                [
                    *qs_reshaped[:, k]
                    + position_noise * np.random.random(qs_reshaped[:, k].shape),
                    *q_dots_reshaped[:, k]
                    + velocity_noise * np.random.random(q_dots_reshaped[:, k].shape),
                    *us_reshaped[:, k]
                    + acceleration_noise * np.random.random(us_reshaped[:, k].shape),
                ]
            )
...
def slow_down_guess_only_ee(
    ...
    starting_ee = mobile_robot.end_effector_pose(
        start_state[: mobile_robot.DoF].full().flatten()
    )
    waypoints_poses, _ = mobile_robot.compute_positions(
        start_state[: mobile_robot.DoF].full().flatten(), [starting_ee, ee_goal]
    )
    dict_graph = mobile_robot.create_graph(waypoints_poses)
