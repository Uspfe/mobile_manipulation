...
def compute_velocities_in_cone_2d(
    points,
    module_min,
    module_max,
    vertex_angle_deg=30,
    ...
    for i, point in enumerate(points_to_inspect):
        ...
        if i < len(points_to_inspect) - 1:
            ...
            if loop:
                next_point = points_to_inspect[
                    0
                ]  # Use initial point for the last point
            else:
                # next point is on the line between the last two points
                next_point = (
                    2 * x - points_to_inspect[i - 1][0],
        ...
        for magnitude in magnitudes:
            ...
            angle = centerline + np.linspace(
                -vertex_angle_rad, vertex_angle_rad, angle_step
            )
...
def compute_velocities_in_cone_3d(
    points,
    module_min,
    module_max,
    vertex_angle_deg=30,
    ...
    for i, point in enumerate(points_to_inspect):
        ...
        centerline_vector = np.array(
            [next_point[0] - x, next_point[1] - y, next_point[2] - z]
        )
        ...
        for magnitude in magnitudes:
            ...
            centerline_phi = np.arctan2(
                np.linalg.norm(centerline_vector[:2]), centerline_vector[2]
            )
            theta = centerline_theta + np.linspace(
                -vertex_angle_rad, vertex_angle_rad, angle_step
            )
            phi = centerline_phi + np.linspace(
                -vertex_angle_rad, vertex_angle_rad, angle_step
            )
            combined_list = []
            if magnitude == 0 or i == 0:
