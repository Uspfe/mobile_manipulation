...
def compute_single_trajectory(
    ...
    for i in range(len(results)):
        if i != max_index:
            extended_trajectory = compute_alpha(
                start_point[i],
                starting_velocity[i],
                end_point[i],
                ending_velocity[i],
...
def compute_trajectory_set(
    points,
    a_max,
    a_min,
    v_max,
    v_min,
    starting_velocity_list,
    ending_velocity_list,
    df,
    start_index,
    end_index,
    prediction_horizon,
    ...
    for i, v0 in enumerate(starting_velocity_list):
        ...
        for j, vf in enumerate(ending_velocity_list):
            params = compute_single_trajectory(
                start_point, end_point, a_max, a_min, v_max, v_min, v0, vf
            )
            if params == None:
                continue
            if start_index == 0 and prediction_horizon == 1:
                if (not f"{end_index}_f" in df[f"{start_index}"].keys()) or (
                    params[0][6] < df[f"{start_index}"][f"{end_index}_f"][0][6]
                ):
                    df[f"{start_index}"][f"{end_index}_f"] = params
                    df[f"{end_index}_f"] = {}
            elif start_index == 0:
                if (not f"{end_index}_{j}" in df[f"{start_index}"].keys()) or (
                    params[0][6] < df[f"{start_index}"][f"{end_index}_{j}"][0][6]
                ):
                    df[f"{start_index}"][
                        f"{end_index}_{j}"
                    ] = params  # create new state for every in between index
            elif start_index == prediction_horizon - 1:
                if (not f"{end_index}_f" in df[f"{start_index}_{i}"].keys()) or (
                    params[0][6] < df[f"{start_index}_{i}"][f"{end_index}_f"][0][6]
                ):
                    df[f"{start_index}_{i}"][f"{end_index}_f"] = params
                    df[f"{end_index}_f"] = {}
            else:
                if (not f"{end_index}_{j}" in df[f"{start_index}_{i}"].keys()) or (
                    params[0][6] < df[f"{start_index}_{i}"][f"{end_index}_{j}"][0][6]
                ):
...
def compute_all_trajectories(
    points, velocity_df, max_acc, min_acc, max_vel, min_vel, prediction_horizon=None
):
    ...
    for i in range(prediction_horizon):
        ...
        trajectories, dict_res = compute_trajectory_set(
            points,
            max_acc,
            min_acc,
            max_vel,
...
def compute_trajectory_set_multi_pose(
    dict_poses,
    start_level,
    end_level,
    start_node,
    ...
    for i, v0 in enumerate(starting_velocity_list):
        if start_level == 0 and i == 0 and not f"{start_level}" in result_dict.keys():
            ...
            start_level != 0
            and not f"{start_level}_{start_node}_{i}" in result_dict.keys()
        ):
            result_dict[f"{start_level}_{start_node}_{i}"] = {}
        for j, vf in enumerate(ending_velocity_list):
            params = compute_single_trajectory(
                start_pose, end_pose, a_max, a_min, v_max, v_min, v0, vf
            )
            if params == None:
                continue
            if start_level == 0 and prediction_horizon == 1:
                if (
                    not f"{end_level}_{end_node}"
                    in result_dict[f"{start_level}"].keys()
                ) or (
                    params[0][6]
                    < result_dict[f"{start_level}"][f"{end_level}_{end_node}"][0][6]
                ):
                    result_dict[f"{start_level}"][f"{end_level}_{end_node}"] = params
                    result_dict[f"{end_level}_{end_node}"] = {}
            elif start_level == 0:
                if (
                    not f"{end_level}_{end_node}_{j}"
                    in result_dict[f"{start_level}"].keys()
                ) or (
                    params[0][6]
                    < result_dict[f"{start_level}"][f"{end_level}_{end_node}_{j}"][0][6]
                ):
                    result_dict[f"{start_level}"][
                        f"{end_level}_{end_node}_{j}"
                    ] = params  # create new state for every in between index
            elif start_level == prediction_horizon - 1:
                if (
                    not f"{end_level}_{end_node}"
                    in result_dict[f"{start_level}_{start_node}_{i}"].keys()
                ) or (
                    params[0][6]
                    < result_dict[f"{start_level}_{start_node}_{i}"][
                        f"{end_level}_{end_node}"
                    ][0][6]
                ):
                    result_dict[f"{start_level}_{start_node}_{i}"][
                        f"{end_level}_{end_node}"
...
def compute_all_trajectories_multi_pose(
    dict_poses, velocity_df, a_max, a_min, v_max, v_min, prediction_horizon=None
):
    ...
    for level in range(prediction_horizon):
        for start_node in range(len(dict_poses[level])):
            ...
            for end_node in range(len(dict_poses[next_level])):
                ...
                trajectories, result_dict = compute_trajectory_set_multi_pose(
                    dict_poses,
                    level,
                    next_level,
                    start_node,
    ...
    if prediction_horizon == len(dict_poses):
        ...
        cost, shortest = shortest_path_multi_pose(
            result_dict, 0, prediction_horizon, dict_poses
        )
