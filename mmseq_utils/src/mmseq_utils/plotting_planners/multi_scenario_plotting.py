...
def main():
    ...
    for file in sorted(os.listdir(args.folder_input)):
        ...
        for file in scenario_files:
            ...
            print(
                f"Time taken from {optimization_type} scenario {i}, initial guess {j}: {initial_guess_time}"
            )
            print(
                f"Time taken from {optimization_type} scenario {i}: {optimization_time}"
            )
        ...
        fig.text(
            0.5,
            0.1,
            f'inverse kinematic computation time: {timing_data["InverseKinematics"][label]["initial_guess_ts"][-1]}, pontryagin computation time: {timing_data["Pontryagin"][label]["initial_guess_ts"][-1]}',
            ha="center",
        )
        ...
        fig.text(
            0.5,
            0.1,
            f'inverse kinematic computation time: {timing_data["InverseKinematics"][label]["optimization_ts"][-1]}, pontryagin computation time: {timing_data["Pontryagin"][label]["optimization_ts"][-1]}',
            ha="center",
        )
        pdf.savefig(fig)
        plt.close(fig)
    ...
    fig = plot_whisker_plots(
        ...
        show=False,
        titles=[
            f"Initialization {optimization_type}",
    ...
    for i in range(len(scenario_labels)):
        ...
        fig = simple_plot(
            ...
            x_label="Scenarios",
            labels=plotting_labels,
            titles=[f"Final Times {optimization_type} {scenario_labels[i]}"],
        )
        pdf.savefig(fig)
        plt.close(fig)
    # overall figgure
    ...
    fig = simple_plot(
        ...
        x_label="Scenarios",
        labels=x_axis,
        titles=[f"Final Times {optimization_type}"],
    )
    pdf.savefig(fig)
    plt.close(fig)
    pdf.close()
