...
def main():
    ...
    # print quick recap
    print(
        f"Time taken from {optimization_type} file {planner.file_name}: {initial_guess_time}"
    )
    print(
        f"Time taken from {optimization_type} file {planner.file_name}: {optimization_time}"
    )
    ...
    tfs = [
        float(X[0]) for X in experiment_data["Scenario " + str(i)][j]["final_solutions"]
    ]
    string = "Final time: "
    ...
    if X_slow is not None:
        ...
        fig = compare_trajectories_casadi_plot(
            ...
            experiment_data["Scenario " + str(i)][j]["ts_slowed"],
            [X_slow_N] * len(experiment_data["Scenario " + str(i)][j]["slow_downed"]),
            q_size=qs_num,
            v_bounds=v_bounds,
            a_bounds=bounds,
            labels=plotting_labels,
