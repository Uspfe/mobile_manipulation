...
def main():
    ...
    parser.add_argument(
        "--folder_input", required=False, help="Folder to save input data"
    )
    ...
    # obstacles
    obstacles_avoidance = (
        planner.motion_class.casadi_model_interface.signedDistanceSymMdlsPerGroup[
            "self"
        ]
    )
    ground_obs = (
    ...
    for file in sorted(
        [file for file in os.listdir(args.folder_input) if f"scenario" in file]
    ):
        tags = file.split("_")
        ...
        scenario_files = sorted(
            [file for file in os.listdir(args.folder_input) if f"scenario_{i}" in file]
        )
        ...
        for (
            ...
            scenario_files
        ):  # assume that files will come ordered by the scaling factor
            tags = file.split("_")
            j = tags[2]
            ...
            experiment_data["Scenario " + str(i)][j]["slow_downed"].append(
                planner.X_slow
            )
            experiment_data["Scenario " + str(i)][j]["ts_slowed"].append(
                planner.X_slow[0]
            )

            # print quick recap
            print(
                f"Time taken from {optimization_type} scenario {i}, initial guess {j}, sf {scaling_factor}: {initial_guess_time}"
            )
            print(
                f"Time taken from {optimization_type} scenario {i}, initial guess {j}, sf {scaling_factor}: {optimization_time}"
            )
            print(
                f"End time scenario {i}, initial guess {j}, sf {scaling_factor}: {X[0]}"
            )
            print()
        ...
        for j in init_configs:
            ...
            fig = compare_trajectories_casadi_plot(
                ...
                None,
                forward_kinematic,
                [9 * 3] * len(experiment_data["Scenario " + str(i)][j]["slow_downed"]),
                experiment_data["Scenario " + str(i)][j]["ts_slowed"],
                [100] * len(experiment_data["Scenario " + str(i)][j]["slow_downed"]),
                q_size=qs_num,
                v_bounds=v_bounds,
                a_bounds=bounds,
                labels=plotting_labels,
            ...
            fig, obstacles_violated = plot_obstacle_avoidance(
                ...
                labels=plotting_labels,
                q_size=qs_num,
                limit=planner.self_collision_safety_margin,
            )
            ...
            fig, objects_violated = plot_obstacle_avoidance(
                experiment_data["Scenario " + str(i)][j]["final_solutions"],
                ground_obs,
                experiment_data["Scenario " + str(i)][j]["total_elements"],
                show=False,
